...
<<<<<<< HEAD
import { SpeedGuardAnimationType } from "../../The_Silent_Mime/AI/SpeedGuard/SpeedGuardStates/SpeedGuardState";
import { DetectiveAnimationType } from "../../The_Silent_Mime/AI/Detective/DetectiveStates/DetectiveState";
=======
import { GuardAnimationType } from "../../The_Silent_Mime/AI/Guard/GuardStates/GuardState";
...
export default abstract class GameNode
  implements Positioned, Unique, Updateable, Physical, Actor, DebugRenderable
{
  /*---------- POSITIONED ----------*/
...
<<<<<<< HEAD
  ...
  guardMoveOnPath(speed: number, path: NavigationPath, owner): void {
    ...
    let x = Math.round(dir.x);
    let y = Math.round(dir.y);
    if (x > 0 && y == 0) {
      owner.animation.playIfNotAlready(SpeedGuardAnimationType.MOVING_RIGHT);
    } else if (x < 0 && y == 0) {
      owner.animation.playIfNotAlready(SpeedGuardAnimationType.MOVING_LEFT);
    } else if (y > 0 && x == 0) {
      owner.animation.playIfNotAlready(SpeedGuardAnimationType.MOVING_DOWN);
    } else if (y < 0 && x == 0) {
      owner.animation.playIfNotAlready(SpeedGuardAnimationType.MOVING_UP);
    }
  }
  ...
  detectiveMoveOnPath(speed: number, path: NavigationPath, owner): void {
    ...
    let y = Math.round(dir.y);
    if (x > 0 && y == 0) {
      owner.animation.playIfNotAlready(DetectiveAnimationType.MOVING_RIGHT);
    } else if (x < 0 && y == 0) {
      owner.animation.playIfNotAlready(DetectiveAnimationType.MOVING_LEFT);
    } else if (y > 0 && x == 0) {
      owner.animation.playIfNotAlready(DetectiveAnimationType.MOVING_DOWN);
    } else if (y < 0 && x == 0) {
      owner.animation.playIfNotAlready(DetectiveAnimationType.MOVING_UP);
    }
  }
  ...
  setTrigger(group: string, onEnter: string, onExit: string): void {
    ...
    if (layerNumber === 0) {
      console.warn(
        `Trigger for GameNode ${this.id} not set - group "${group}" was not recognized by the physics manager.`
      );
      return;
    }
  ...
  debugRender(): void {
    ...
    if (this._velocity && !this._velocity.isZero()) {
      Debug.drawRay(
        ...
        this._velocity.clone().scaleTo(20).add(this.relativePosition),
        Color.BLUE
      );
    }
...
=======
    ...
    let y = Math.round(dir.y);
    if (x > 0 && y == 0) {
      owner.animation.playIfNotAlready(GuardAnimationType.MOVING_RIGHT);
    } else if (x < 0 && y == 0) {
      owner.animation.playIfNotAlready(GuardAnimationType.MOVING_LEFT);
    } else if (y > 0 && x == 0) {
      owner.animation.playIfNotAlready(GuardAnimationType.MOVING_DOWN);
    } else if (y < 0 && x == 0) {
      owner.animation.playIfNotAlready(GuardAnimationType.MOVING_UP);
    }
  }
...
>>>>>>> 7106fa920dadf71748d39a1a4b858fe73fb4bbd2
      ...
      if (this.collisionShape instanceof AABB) {
        Debug.drawBox(
          this.inRelativeCoordinates(this.collisionShape.center),
          this.collisionShape.halfSize.scaled(this.scene.getViewScale()),
          true,
          color
        );
        ...
        Debug.drawCircle(
          this.inRelativeCoordinates(this.collisionShape.center),
          this.collisionShape.hw * this.scene.getViewScale(),
          true,
          color
        );
      }
